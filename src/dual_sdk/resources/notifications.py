...
class Notifications(SyncResource):
    ...
    def create_template(self, *, name: str, body: str, **fields: Any) -> MessageTemplate:
        ...
        return _parse(
            MessageTemplate,
            self._post("/messages/templates", json={"name": name, "body": body, **fields}),
        )

    def update_template(self, template_id: str, **fields: Any) -> MessageTemplate:
        """Update a message template."""
        return _parse(
            MessageTemplate, self._patch(f"/messages/templates/{template_id}", json=fields)
        )
...
class AsyncNotifications(AsyncResource):
    ...
    async def send(self, *, content: str, **fields: Any) -> Message:
        return _parse(
            Message, await self._post("/messages/send", json={"content": content, **fields})
        )
    ...
    async def create_template(self, *, name: str, body: str, **fields: Any) -> MessageTemplate:
        return _parse(
            MessageTemplate,
            await self._post("/messages/templates", json={"name": name, "body": body, **fields}),
        )

    async def update_template(self, template_id: str, **fields: Any) -> MessageTemplate:
        return _parse(
            MessageTemplate, await self._patch(f"/messages/templates/{template_id}", json=fields)
        )
